...
@Component({
  ...
  <div class="container">
    <nb-card class="">
        <h6 class="display-4 p-3">Active Trips</h6>
    </nb-card>
    <div>
...
export class TripActiveComponent implements OnInit {
    ...
    listActiveTrips() {
        ...
        const tripsObservable = this.tripService.listActiveTrips();

        tripsObservable.subscribe((trips) => {
            this.tripsData = trips.filter(driver => {
                return driver.AmountTendered > 0;
            });
